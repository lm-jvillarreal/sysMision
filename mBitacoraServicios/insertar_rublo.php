<?php
    ...
    if($id_registro_modal == 0){
        $verificar = mysqli_query($conexion,"SELECT id FROM rublos WHERE nombre = '$rublo_modal' AND activo = '1'");
        $cantidad  = mysqli_num_rows($verificar);
        
        if ($cantidad == 0){
            $cadena = "INSERT INTO rublos (nombre,fecha,hora,activo,id_usuario)
                    VALUES ('$rublo_modal','$fecha','$hora','1','$id_usuario')";
