<?php
	...
	if($servicio_id == 0){
		$cadena = mysqli_query($conexion,"INSERT INTO bitacora_servicios (id_proveedor,nombre_encargado,fecha_servicio,gasto,supervisor,id_rublo,fecha,hora,activo,id_usuario,comentario,id_sucursal) VALUES('$id_proveedor','$encargado','$fecha1','$gasto','$supervisor','$rublo','$fecha','$hora','1','$id_usuario','$comentario','$id_sucursal')");

		$cadena_id = mysqli_query($conexion,"SELECT MAX(id) FROM bitacora_servicios");
		...
		if (empty($_FILES['file-es'])) {
		 ...
		 //    return; 
			...
			define('DIR_DESCARGAS',__DIR__.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.$row_id[0]);
		...
		$cadena = mysqli_query($conexion,"UPDATE bitacora_servicios SET id_proveedor = '$id_proveedor',nombre_encargado = '$encargado',fecha_servicio = '$fecha1',gasto = '$gasto',supervisor = '$supervisor',id_rublo = '$rublo',comentario = '$comentario', id_sucursal = '$id_sucursal' WHERE id = '$servicio_id'");
		$respuestas = "ok";
		if (empty($_FILES['file-es'])) {
			// Devolvemos un array asociativo con la clave error en formato JSON como respuesta	
		 //    echo json_encode(['error'=>'No hay ficheros para realizar upload.']); 
		 ...
		 //    return; 
			...
			// Definimos la constante con el directorio de destino de las descargas
			define('DIR_DESCARGAS',__DIR__.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.$servicio_id);
			// Obtenemos el array de ficheros enviados
			$ficheros = $_FILES['file-es'];
			// Establecemos el indicador de proceso correcto (simplemente no indicando nada)
			$estado_proceso = NULL;
