<?php 
  ...
  $cadena   = "SELECT
                id_promotor,
                (
                  SELECT CONCAT( promotores.nombre,' ',promotores.ap_paterno)
                  FROM promotores
                  WHERE promotores.id = registro_entrada.id_promotor
                ) AS Promotor,
                (
                  SELECT compañia
                  FROM promotores
              ...
              WHERE hora_salida is null AND id_sucursal = '$id_sede'";
  ...
  while ($row = mysqli_fetch_array($consulta)) 
  {
    ...
    $cadena2 = mysqli_query($conexion,"SELECT actividades_promotor.actividad,registro_actividades.hora_inicio
              FROM registro_actividades
              INNER JOIN actividades_promotor ON actividades_promotor.id = registro_actividades.id_actividad  
              WHERE registro_actividades.fecha = '$fecha_actual' 
              ...
              AND registro_actividades.id_sucursal = '$id_sede'");
