...
$cadena_folios = "SELECT DISTINCT(m.folio), m.folio_desc, count(m.id), m.fecha, m.hora, u.nombre_usuario, sucursales.nombre
									FROM auditoria_pv as m inner join usuarios as u ON m.usuario = u.id inner join sucursales ON m.sucursal = sucursales.id
									WHERE m.folio_desc is not null 
									and m.usuario = '$id_usuario'
									group by m.folio";
