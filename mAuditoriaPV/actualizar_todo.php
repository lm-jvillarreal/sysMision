...
$cadenaConsulta = "SELECT id, articulo, sucursal FROM auditoria_pv WHERE usuario = '100' and fecha = '2019-12-13' LIMIT 1";
...
while($rowCodigo = mysqli_fetch_array($consultaCodigo)){
  ...
  if ($conteo==0) {
    ...
    $existencia = (empty($row_articulo[5])) ? 'NULL' : $row_articulo[5];
    $cantidad_mov =  (empty($row_ue[3])) ? 'NULL' : $row_ue[3];

    $cadenaActualizar = "UPDATE auditoria_pv SET descripcion = '$row_articulo[1]', existencia = $existencia, ultima_entrada = '$row_ue[1]', tipo_mov = '$row_ue[2]', folio_mov = '$row_ue[0]', cantidad_mov = $cantidad_mov, proveedor = '$row_ue[5]', departamento = '$row_articulo[4]', familia = '$row_articulo[3]', ultimo_costo = '$ultimoCosto', unidad_empaque = '$row_articulo[7]', ventas = '$row_ventas[0]', teorico = '$row_articulo[5]', faltante = '$faltante', faltante_cajas = '$fue', dias_inv = '$dias_inventario', meses_inv = '$mes_inventario' WHERE id = '$rowCodigo[0]'";
