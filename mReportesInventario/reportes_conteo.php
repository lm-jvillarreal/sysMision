...
$qry = "SELECT 
				IFNULL((SELECT 
						...
						FROM
								inv_captura
						WHERE
								cod_producto = detalle.codigo_producto
										AND id_mapeo = detalle.id_mapeo
						GROUP BY cod_producto),0),
						detalle.codigo_producto
				FROM
				inv_mapeo AS mapeo
						INNER JOIN
				inv_detalle_mapeo AS detalle ON mapeo.id = detalle.id_mapeo
				WHERE
				mapeo.fecha_conteo = '$fecha_conteo'
						AND mapeo.activo = '0'
						AND mapeo.id_sucursal = '$sucursal'";
			//echo "$qry";
$consulta_proveedor=mysqli_query($conexion, $qry);
	...
	while($row = mysqli_fetch_array($consulta_proveedor))
	{
		...
		$qry_datos = "SELECT 
						...
						END CASE,
						(SELECT ARTN_ULTIMOPRECIO FROM COM_ARTICULOS WHERE ARTC_ARTICULO = '$row[1]') AS U_P
						FROM PV_ARTICULOS
						WHERE ARTC_ARTICULO = '$row[1]'";
		...
		$objPHPExcel->setActiveSheetIndex(0)
	            ...
	            ->setCellValue('C'.$fila, $row[0])
	            ->setCellValue('D'.$fila, $row_existencia[1])
	            ->setCellValue('E'.$fila, $row[0] - $row_existencia[1])
	            ->setCellValue('F'.$fila, $row_datos[5])
	            ->setCellValue('G'.$fila, $row_datos[4])
	            ->setCellValue('H'.$fila, ($row[0]-$row_existencia[1]) * $row_datos[5])
	            ->setCellValue('I'.$fila, ($row[0]-$row_existencia[1]) * $row_datos[4]);
