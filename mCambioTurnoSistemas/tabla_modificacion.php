...
$solo_suc = ($solo_sucursal == '1') ? " AND sucursal='$id_sede'" : "";
  ...
  $cadena  = "SELECT id, 
              ...
              FROM cambio_turnoSistemas WHERE ACTIVO = '1'AND estatus = '0'".$solo_suc;
  ...
  {
    $cadena_perfil="SELECT id, id_perfil from usuarios where activo = '1' AND id = '$id_usuario'";
    $consulta_perfil = mysqli_query($conexion, $cadena_perfil);
    $row_perfil = mysqli_fetch_array($consulta_perfil);
    if($row_perfil[1] == "11"){
      //echo($row_perfil[1]);
      $autorizar = "<center><a href='#' class='btn btn-aqua disabled'><i class='fa fa-check fa-lg'></a></center>";
    }else{
      //echo($row_perfil[1]);
