...
$cadenaRecetas = "SELECT r.id, r.codigo_receta, r.nombre_receta, (r.rendimiento), ((100-r.costo_operativo)/100), ((100-r.margen_sugerido)/100),
                  ...
                  GROUP BY r.id ";
$consultaReceta = mysqli_query($conexion,$cadenaRecetas);
while($rowReceta = mysqli_fetch_array($consultaReceta)){
  ...
  $cadenaInsertar = "INSERT INTO cp_resumen_receta (id_receta, codigo_receta, nombre_receta, rendimiento, margen_operativo, margen_sugerido, subtotal, total, costo, precio_sugerido, pp_actual, margen_actual, fecha, hora, activo, usuario,costo_ingredientes)VALUES('$rowReceta[0]', '$rowReceta[1]', '$rowReceta[2]', '$rendimiento', '$moReal', '$msReal', '$subtotal', '$stOperativo', '$costo', '$pp_sugerido', '$pp_actual', '$margen_actual', '$fecha', '$hora', '1', '$id_usuario','$costoIngredientes')";
