...
WHERE
  MONTH ( notas_entrada.fecha ) = MONTH ( CURRENT_DATE )";
//echo "$qry";
    ...
    <body>
      ...
      while ($row = mysqli_fetch_row($exQry)) {
        ...
        <tr>
          ...
          <td><a href="#" class="btn btn-danger" onclick="javascript:ver('<?php echo $row[8] ?>', '<?php echo $row[1] ?>', '<?php echo $row[9] ?>')"><i class='fa fa-file-text fa-lg'></i></a></td>
          <td><a href="#" class="btn btn-danger" onclick="javascript:editar_dato(<?php echo $row[10] ?>, '<?php echo $row[5] ?>', '<?php echo $row[2] ?>', '<?php echo $row[1] ?>', '<?php echo $row[8] ?>', <?php echo $row[3] ?>, <?php echo $row[0] ?>, <?php echo $row[9] ?>)"><i class='fa fa-edit fa-lg'></i></a></td>
