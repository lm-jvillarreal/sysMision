<?php 
  ...
  $verificar = mysqli_query($conexion,"SELECT id, hora_inicio FROM registro_actividades WHERE id_actividad = '$id_actividad' AND fecha = '$fecha'");
  $existe = mysqli_num_rows($verificar);
  $row    = mysqli_fetch_array($verificar);

  if($existe == 0){
    $cadena_registrar = mysqli_query($conexion,"INSERT INTO registro_actividades (id_actividad, hora_inicio, fecha, hora, activo, id_usuario) VALUES('$id_actividad','$hora','$fecha','$hora','1','$id_usuario')");
    $mensaje = "ok";
  }else{
