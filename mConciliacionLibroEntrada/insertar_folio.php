...
if($modc_tipomov=='ENTCOC' || $modc_tipomov=='ENTSOC'){
  $cadenaMonto = "SELECT ARTC_ARTICULO, rmon_costo_renglon_mb, rmon_ieps_sn, rmoc_cveieps, RMOC_CVEIVA
  ...
  $cadenaMonto="SELECT (SELECT NC.NCCN_IMPORTE FROM cxp_notascarcre NC WHERE nc.nccv_numncc = inv_movimientos.movc_cxp_remision) TOTAL
