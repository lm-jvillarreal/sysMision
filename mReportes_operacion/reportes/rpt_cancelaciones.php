...
$fechados = date_format($date2, 'd-m-Y');
	...
	while(strtotime($fechados) >= strtotime($fechauno))
	{
		if(strtotime($fechados) != strtotime($fechaaamostar))
		{
			...
			INNER JOIN CTB_USUARIO ON CTB_USUARIO.USUS_USUARIO = PV_ACUMULADOS.CCAC_CAJERO
			...
			while ($row = oci_fetch_row($st)) {
				...
				$insert = "INSERT INTO sobrantes_faltantes (
							...
							VALUES
								('$fechaaamostar', '$sobrante_total', '$faltante_total', '$row[0]', '$row[3]')";
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ...
	            ->setCellValue('C1', 'Sobrante Total')
	            ->setCellValue('D1', 'Faltante Total');
	...
	header('Content-Disposition: attachment;filename="Faltantes y sobrantes" '.$Msucursal.$fecha.' ".xlsx"');
