...
$cadena_consulta = "SELECT  ID, ARTC_ARTICULO, ARTC_DESCRIPCION, CANTIDAD_SOLICITA, CANTIDAD_SURTIDA, FOLIO_TRASPASO, FOLIO_PEDIDO 
                    FROM solicitud_traspasos 
                    WHERE date_format(FECHAHORA_SOLICITA,'%Y-%m-%d')>='2020-07-01' 
                    AND date_format(FECHAHORA_SOLICITA,'%Y-%m-%d')<='2020-07-31'
                    AND !isnull(CANTIDAD_SURTIDA) AND !isnull(FOLIO_TRASPASO)
                    AND ESTATUS='3'";
...
while($rowDetalle=mysqli_fetch_array($consulta_detalle)){
  ...
  $cadenaFinaliza="UPDATE solicitud_traspasos SET ARTC_ENVIADO='$artc_enviado', ARTC_DIFERENCIAPEDIDO='$artc_diferenciapedido', ARTC_DIFERENCIATRASPASO='$artc_diferenciatraspaso', CANTIDAD_RECIBIDA='$artc_cantidadrecibida', ESTATUS='4' WHERE ID='$rowDetalle[0]'";
  $finalizaPedido=mysqli_query($conexion,$cadenaFinaliza);
  echo $cadenaFinaliza."<br>";
}
?>