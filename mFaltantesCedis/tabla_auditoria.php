...
$cadena="SELECT DISTINCT(FOLIO_PEDIDO), DESCRIPCION_TRASPASO, DATE_FORMAT(FECHAHORA_SOLICITA,'%d/%m/%Y'),
        ...
        WHERE (T.ESTATUS='3') AND T.SUCURSAL=$id_sede";
...
while ($row = mysqli_fetch_array($consulta)) 
{
  if($row[5]=='1'){
    ...
    $link = "<center><a href='#' data-folio = '$row[0]' data-salida = '$row[7]' data-toggle = 'modal' data-target = '#modal-detalle' class='btn btn-primary' target='blank'><i class='fa fa-search fa-lg' aria-hidden='true'></i></a></center>";
  }elseif($row[5]=='4'){
