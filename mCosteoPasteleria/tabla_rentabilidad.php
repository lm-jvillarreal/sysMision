...
$cadenaRecetas="SELECT ID, ARTC_ARTICULO, ARTC_DESCRIPCION FROM pasteleria_recetasventa WHERE ACTIVO=1";
...
while($rowRecetas=mysqli_fetch_array($consultaRecetas)){
  ...
  while($rowDetalle=mysqli_fetch_array($consultaDetalle)){
    if($rowDetalle[1]=='1'){
      ...
      $cadenaSR="SELECT ID, UNIDAD_MEDIDA, RENDIMIENTO AS FACTOR_EMPAQUE FROM pasteleria_subrecetas WHERE CLAVE_RECETA='$rowDetalle[0]'";
      ...
      $cadenaDesglose="SELECT ID, ID_ARTICULO, CANTIDAD_RECETA, SUBRECETA FROM pasteleria_subrecetasrenglones WHERE ID_SUBRECETA='$rowSR[0]' AND ACTIVO='1'";
      //echo $cadenaDesglose;
      ...
      $cadenaArtc="SELECT
                  ...
                  WHERE
	                r.CLAVE_ARTICULO = '$rowDetalle[0]' AND r.ID_PRODUCTO='$rowRecetas[0]'";
      //echo $cadenaArtc;
