...
$cadenaDetalle="SELECT
                  ...
                  MERMA 
                  FROM
                  pasteleria_recetasventarenglones 
                  WHERE
                  ACTIVO = 1 
...
while($rowDetalle=mysqli_fetch_array($consultaDetalle)){
  ...
  if($rowDetalle[4]=='1'){
    ...
    $cadenaSR="SELECT CLAVE_RECETA, NOMBRE_RECETA, UNIDAD_MEDIDA, RENDIMIENTO AS FACTOR_EMPAQUE, ID FROM pasteleria_subrecetas WHERE CLAVE_RECETA='$rowDetalle[2]'";
    ...
    $cadenaDesglose="SELECT ID, ID_ARTICULO, CANTIDAD_RECETA, SUBRECETA, ACTIVO FROM pasteleria_subrecetasrenglones WHERE ID_SUBRECETA='$rowSR[4]' AND ACTIVO='1'";
    ...
    while($rowDesglose=mysqli_fetch_array($consultaDesglose)){
      ...
      if($rowDesglose[3]=='1'  && $rowDesglose[4] == '1'){
        ...
        $cadenaRec="SELECT ID, NOMBRE_RECETA, RENDIMIENTO, UNIDAD_MEDIDA FROM pasteleria_subrecetas WHERE CLAVE_RECETA='$rowDesglose[1]'";
        ...
        $cadenaTotales="SELECT
                        ...
                        ( SELECT RENDIMIENTO FROM pasteleria_subrecetas WHERE ID = '$rowRec[0]' ) AS RENDIMIENTO,
                        round(
                          (
                            SUM( ( ( a.RMON_ULTIMOPRECIO / a.FACTOR_EMPAQUE ) * r.CANTIDAD_RECETA ) / ( 1- ( a.PORCENTAJE_MERMA / 100 ) ) ) + SUM( ( ( a.RMON_ULTIMOPRECIO / a.FACTOR_EMPAQUE ) * r.CANTIDAD_RECETA ) / ( 1- ( a.PORCENTAJE_MERMA / 100 ) ) ) * 0.03 
                          ) / ( SELECT RENDIMIENTO FROM pasteleria_subrecetas WHERE ID = '$rowRec[0]' ),
                          2 
                        ) AS COSTO_UM,
                        a.UNIMEDIDA_VENTA 
                      FROM
                        pasteleria_subrecetasrenglones AS r
                        INNER JOIN pasteleria_articulos AS a ON r.ID_ARTICULO = a.ARTC_ARTICULO 
                      WHERE
    ...
    $activo = "<center><a href='#' class='btn btn-danger'onclick ='cambiarEstado($articulo,$producto,$ide)'><i class='fa fa-trash fa-lg' aria-hidden='true'></i></a></center>";
  }else{
    ...
    $cadenaArtc="SELECT
                  ...
                  FROM
                  pasteleria_recetasventarenglones AS r
                  INNER JOIN pasteleria_articulos AS a ON r.CLAVE_ARTICULO = a.ARTC_ARTICULO
