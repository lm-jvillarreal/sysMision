...
$cadenaRecetas="SELECT ID, ARTC_ARTICULO, ARTC_DESCRIPCION FROM perecederos_recetasventa WHERE ACTIVO=1";
...
while($rowRecetas=mysqli_fetch_array($consultaRecetas)){
  ...
  while($rowDetalle=mysqli_fetch_array($consultaDetalle)){
    if($rowDetalle[1]=='1'){
      ...
      $cadenaSR="SELECT ID, UNIDAD_MEDIDA, RENDIMIENTO AS FACTOR_EMPAQUE FROM perecederos_subrecetas WHERE CLAVE_RECETA='$rowDetalle[0]'";
      ...
      $cadenaDesglose="SELECT ID, ID_ARTICULO, CANTIDAD_RECETA, SUBRECETA FROM perecederos_subrecetasrenglones WHERE ID_SUBRECETA='$rowSR[0]' AND ACTIVO='1'";
      ...
      $merma=$rowDetalle[2];
      if($costo_empaque>0 && $factor_empaque>0){
      ...
      $cadenaArtc="SELECT
                      ...
                      WHERE r.CLAVE_ARTICULO ='$rowDetalle[0]' AND r.ID_PRODUCTO='$rowRecetas[0]'";
