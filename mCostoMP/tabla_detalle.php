...
$cadenaDesglose="SELECT ID, ID_ARTICULO, CANTIDAD_RECETA, SUBRECETA, ACTIVO FROM perecederos_subrecetasrenglones WHERE ID_SUBRECETA='$id_receta' AND ACTIVO='1'";
...
while($rowDesglose=mysqli_fetch_array($consultaDesglose)){
  if($rowDesglose[3]=='1' && $rowDesglose[4] == '1'){
    ...
    $cadenaReceta="SELECT
              ...
              WHERE
                r.ID_SUBRECETA = '$rowRec[0]' AND r.ACTIVO = '1'";
    ...
    $codigo = json_encode($rowDesglose[1]);
    $activo = "<center><a href='#' class='btn btn-danger'onclick ='cambiarEstado($codigo, $id_receta,$ide)'><i class='fa fa-trash fa-lg' aria-hidden='true'></i></a></center>";
