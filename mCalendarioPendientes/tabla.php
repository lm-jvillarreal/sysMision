<?php 
    ...
    $cadena  = "SELECT id,pendiente, DATE_FORMAT(fecha_inicial, '%d-%m-%Y'), DATE_FORMAT(fecha_final, '%d-%m-%Y'), (SELECT nombre FROM  tipo_actividad_mantenimiento WHERE tipo_actividad_mantenimiento.id = pendientes_mantenimiento.id_tipo_actividad), (SELECT CONCAT(nombre, ' ', ap_paterno, ' ', ap_materno) FROM usuarios INNER JOIN personas ON personas.id = usuarios.id_persona WHERE usuarios.id = pendientes_mantenimiento.id_usuario_asignado) FROM pendientes_mantenimiento WHERE activo = '1'".$filtro;
