	...
	if(isset($_POST['impuesto'])){
  	...
  	if(!empty($_POST['id_registro'])){
		...
		if($row_impuestos[1] == $ieps){
  		...
  		$cadena_actualizar = mysqli_query($conexion,"UPDATE altas_productos SET id_comprador = '$comprador', id_proveedor = '$proveedor', costo = '$costo ', clave_sat = '$clave_sat', descuento1 ='$descuento1', descuento2 = '$descuento2', costo_final = '$costo_final'".$cadena_iva.' '.$cadena_ieps." WHERE id = '$id_registro'");
		...
		$cadena = mysqli_query($conexion,"INSERT altas_productos (id_proveedor,id_comprador,folio,costo,iva,ieps,estatus,fecha,hora,id_usuario,activo,id_sucursal,clave_sat,descuento1,descuento2,costo_final)
					VALUES('$proveedor','$comprador','$folio','$costo','$iva','$ieps','0','$fecha','$hora','$id_usuario','1','$id_sede','$clave_sat','$descuento1','$descuento2','$costo_final')");
		echo "ok";
  	}
 ?>