...
$consulta_principal  = "SELECT 
                            ...
                            (
                            SELECT 
                                spin_articulos.fn_existencia_disponible_todos ( 13, NULL, NULL, 1, 1, 202, COM_ARTICULOS.ARTC_ARTICULO)
                            FROM 
                                dual
                            ) ARB,
                            (SELECT  
                                spin_articulos.fn_existencia_disponible_todos ( 13, NULL, NULL, 1, 1, 402, COM_ARTICULOS.ARTC_ARTICULO)
                            FROM 
                                dual) ALL,
                            (SELECT 
                                spin_articulos.fn_existencia_disponible_todos ( 13, NULL, NULL, 1, 1, 502, COM_ARTICULOS.ARTC_ARTICULO)
                                
                            FROM 
                                dual) PET,
							(SELECT 
                                spin_articulos.fn_existencia_disponible_todos ( 13, NULL, NULL, 1, 1, 602, COM_ARTICULOS.ARTC_ARTICULO)
	...
	header('Content-Disposition: attachment;filename="REPCOM_ExiSuc_Restaurant'.date('Ymd').'.xlsx"');
