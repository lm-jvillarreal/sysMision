...
$consulta_principal  = "SELECT 
                            ...
                            (
                            SELECT 
                                spin_articulos.fn_existencia_disponible_todos ( 13, NULL, NULL, 1, 1, 100, COM_ARTICULOS.ARTC_ARTICULO)
                            FROM 
                                dual
                            ) DO,
                            (SELECT 
                                        
                                        spin_articulos.fn_existencia_disponible_todos ( 13, NULL, NULL, 1, 1, 200, COM_ARTICULOS.ARTC_ARTICULO)
                                    FROM 
                                        dual) ARB,
                            (SELECT 
                                        spin_articulos.fn_existencia_disponible_todos ( 13, NULL, NULL, 1, 1, 300, COM_ARTICULOS.ARTC_ARTICULO)
                                        
                                    FROM 
                                        dual) VILL,
                            (SELECT 
                                        spin_articulos.fn_existencia_disponible_todos ( 13, NULL, NULL, 1, 1, 400, COM_ARTICULOS.ARTC_ARTICULO)
                                    FROM 
                                        dual) ALLE,
                            (SELECT 
                                        spin_articulos.fn_existencia_disponible_todos ( 13, NULL, NULL, 1, 1, 500, COM_ARTICULOS.ARTC_ARTICULO)
                                    FROM 
                                        dual) PET,
							(SELECT 
                                        spin_articulos.fn_existencia_disponible_todos ( 13, NULL, NULL, 1, 1, 600, COM_ARTICULOS.ARTC_ARTICULO)
                                    FROM 
                                        dual) MM,
	...
	header('Content-Disposition: attachment;filename="REPCOM_ExiSucSep_ARTACT'.date('Ymd').'.xlsx"');
