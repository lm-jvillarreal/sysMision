<?php
	...
	if (empty($id_registro)) {
		...
		$verificar=mysqli_query($conexion,"SELECT id FROM actas_incidencias WHERE id= '$id_registro'");
		$existe = mysqli_num_rows($verificar);
		// $cant      = mysqli_num_rows($existe);
		if ($existe == 0) 
		{
			$cadena= "INSERT INTO actas_incidencias( nombre, fecha, hora, activo, usuario)
			VALUES ('$nombre', '$fecha','$hora','1','1')";

			$consulta = mysqli_query($conexion,$cadena);
			
			//Copia de firma
			$cadena_maximo = "SELECT MAX(id) FROM actas_incidencias";
			$consulta_maximo = mysqli_query($conexion, $cadena_maximo);
			$row_maximo = mysqli_fetch_array($consulta_maximo);
			$destino = "actas/".$row_maximo[0].".".$extension;
