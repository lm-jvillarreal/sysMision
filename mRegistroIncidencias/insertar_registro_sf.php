...
if($categoria == 'Indisciplina'){
  ...
  if (empty($id_registro)) {
    ...
    if($existe == 0){
      $cadena_insertar = "INSERT INTO incidencias (nombre,departamento,sucursal,incidencia,fecha,hora,activo, usuario, folio, comentario, categoria,tipo, accion, autorizacion, comentario_fin,vigilante,perfil, sucursal2)
      VALUES('$nombre','$departamento','$sucursal','$incidencia','$fecha','$hora', '1', '$id_usuario', '0','$comentario', '$categoria','$tipo','$accion','0','$firma','$vigilante','$perfil','$sucursal2')";
    ...
    $row_incidencias = mysqli_fetch_array($cadena_incidencias);    
     ...
     $title = $add.'-'.$title.'-'.$sucu;
        ...
        $cadena_folio = mysqli_query($conexion,"SELECT MAX(folio) FROM agenda");
         ...
         $cadena_eventos = mysqli_query($conexion,"SELECT
         usuarios.id,
...
/////////////////////////////////////////notificaciones////////////////////////////////////////////////////////
    ...
    $cadena_actualizar = "UPDATE incidencias SET nombre = '$nombre', departamento='$departamento',sucursal= '$sucursal',categoria='$categoria',tipo='$tipo', incidencia='$incidencia', comentario='$comentario', accion='$accion', vigilante='$vigilante', usuario='$id_usuario' WHERE id = '$id_registro'";
