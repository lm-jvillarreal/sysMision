...
//echo $lugar;
  ...
  $cadena  = "SELECT i.id, i.nombre, i.empleado, i.departamento, ci.incidencia, i.folio, i.activo, i.comentario, i. perfil, i.usuario, CONCAT(p.nombre,' ',p.ap_paterno,' ', p.ap_materno),perfil.nombre, DATE_FORMAT(i.fecha,'%d/%m/%Y'), i.sucursal
  FROM incidencias i INNER JOIN catalogo_incidencias ci, personas p, perfil, usuarios 
  WHERE i.fecha BETWEEN CAST('$dateI'AS DATE) AND CAST('$dateF' AS DATE) AND i.incidencia = ci.id AND i.activo = '1' AND i.usuario = usuarios.id AND p.id = usuarios.id_persona AND perfil.id=i.perfil AND i.empleado IS NOT NULL".$filtro;
//  sucursal2='$id_sede'
  ...
  while ($row_incidencias = mysqli_fetch_array($consulta)) 
  {
    ...
    $empleado = $row_incidencias[1].' - '.$row_incidencias[2];
