<?php 
	...
	$qry_traspasos = "SELECT 
		...
		$qry_ventas="SELECT NVL(SUM(DETALLE.ARTN_CANTIDAD),'0')
									FROM
											PV_ARTICULOSTICKET detalle
									INNER JOIN PV_TICKETS tik ON CONCAT(TIK.TICN_AAAAMMDDVENTA,TIK.TICN_FOLIO) = CONCAT(DETALLE.TICN_AAAAMMDDVENTA,DETALLE.TICN_FOLIO)
									AND tik.TICC_SUCURSAL=detalle.ticc_sucursal
	...
	$qry_mermas = "SELECT 
		...
		$sql_sirota = "SELECT sum(rm.rmon_cantidad) FROM INV_MOVIMIENTOS m
		...
		$st_sirota = oci_parse($conexion_central, $sql_sirota);
		oci_execute($st_sirota);
		$row_sirota = oci_fetch_row($st_sirota);
		if (is_null($row_sirota[0])) {
			$row_sirota[0] = 0;
		}else{
			$row_sirota[0] = $row_sirota[0];
		}
		///////////////////////////////
		$array_datos = array(
			$row_compras[0],//0 
