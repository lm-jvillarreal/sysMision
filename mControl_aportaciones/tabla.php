...
$cadena_aportaciones = "SELECT 
							...
							cve_proveedor
						FROM 
							aportaciones WHERE (concepto='APERTURA MONTEMORELOS') AND anio='$anio'";
...
while ($row_aportaciones = mysqli_fetch_array($consulta_aportaciones)) {
	...
	if ($row_aportaciones[1]=="NC") {
		...
		$link_detalle = "<center><a href='#' data-id = '$row_aportaciones[2]' data-mov = '$row_aportaciones[1]' data-suc = '$row_aportaciones[4]' data-toggle = 'modal' data-target = '#modal-default' class='btn btn-danger' target='blank'>$row_aportaciones[2]</a></center>";
