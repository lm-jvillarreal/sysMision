...
$cadena_consulta = "SELECT ARTC_DESCRIPCION, artn_precioventa, artc_tipoimpuesto1, artc_tipoimpuesto2, artc_tipoimpuesto3 FROM PV_ARTICULOS WHERE ARTC_ARTICULO = '$p_codigo'";
...
if ($cantidad_articulos==0) {
	...
	$cadena_ofertas = "SELECT pv_configuracion_oferta.coon_tipo, pv_articulos_oferta.aron_porcdescuentooprecio,
						to_char(pv_configuracion_oferta.cood_vigencia_fin,'DD/MM/YYYY'),
						to_char(pv_configuracion_oferta.cood_vigencia_fin,'YYYY-MM-DD'),
						pv_articulos_oferta.aroc_sucursal,pv_configuracion_oferta.cooc_descripcion
						FROM pv_configuracion_oferta 
						INNER JOIN pv_articulos_oferta 
						ON pv_configuracion_oferta.coon_id_oferta = pv_articulos_oferta.coon_id_oferta
						AND pv_configuracion_oferta.cood_vigencia_ini <= TO_DATE('$fecha','dd/mm/yyyy')
						AND pv_configuracion_oferta.cood_vigencia_fin >= TO_DATE('$fecha','dd/mm/yyyy')
						AND pv_configuracion_oferta.COON_AUTORIZADO_SN='1'
						AND pv_articulos_oferta.aroc_articulo = '$p_codigo'
						AND pv_articulos_oferta.aroc_sucursal = '1'
						ORDER BY pv_articulos_oferta.aron_porcdescuentooprecio ASC";
	//echo $cadena_ofertas;
