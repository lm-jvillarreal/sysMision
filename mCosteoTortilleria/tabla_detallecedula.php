...
while($rowDetalle=mysqli_fetch_array($consultaDetalle)){
  ...
  if($rowDetalle[4]=='1'){
    ...
    $cadenaSR="SELECT CLAVE_RECETA, NOMBRE_RECETA, UNIDAD_MEDIDA, RENDIMIENTO AS FACTOR_EMPAQUE, ID FROM tortilleria_subrecetas WHERE CLAVE_RECETA='$rowDetalle[2]'";
    ...
    $cadenaDesglose="SELECT ID, ID_ARTICULO, CANTIDAD_RECETA, SUBRECETA, ACTIVO FROM tortilleria_subrecetasrenglones WHERE ID_SUBRECETA='$rowSR[4]' AND ACTIVO='1'";
    ...
    $cadenaArtc="SELECT
                ...
                tortilleria_subrecetasrenglones AS r
                INNER JOIN tortilleria_articulos AS a ON r.ID_ARTICULO = a.ARTC_ARTICULO
                WHERE r.ID_ARTICULO='$rowDetalle[2]'";
    ...
    $prod = json_encode($producto);
    $activo = "<center><a href='#' class='btn btn-danger'onclick ='cambiarEstado($articulo,$prod)'><i class='fa fa-trash fa-lg' aria-hidden='true'></i></a></center>";
  }
