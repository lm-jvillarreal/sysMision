...
$cadenaDesglose="SELECT ID, ID_ARTICULO, CANTIDAD_RECETA, SUBRECETA, ACTIVO FROM tortilleria_subrecetasrenglones WHERE ID_SUBRECETA='$id_receta' AND ACTIVO='1'";
...
while($rowDesglose=mysqli_fetch_array($consultaDesglose)){
  if($rowDesglose[3]=='1' && $rowDesglose[4] == '1'){
    ...
    $activo = "<center><a href='#' class='btn btn-success'onclick ='cambiarEstado($articulo,$id_receta)'><i class='fa fa-pencil fa-lg' aria-hidden='true'></i></a></center>";
    ...
    $articulo = json_encode($rowDesglose[1]);
    $activo = "<center><a href='#' class='btn btn-danger'onclick ='cambiarEstado($articulo,$id_receta)'><i class='fa fa-trash fa-lg' aria-hidden='true'></i></a></center>";
  }
