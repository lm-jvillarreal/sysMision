...
$cadena_consulta  = "SELECT 
   (SELECT FAMC_DESCRIPCION FROM COM_FAMILIAS WHERE FAM.FAMC_FAMILIAPADRE = COM_FAMILIAS.FAMC_FAMILIA AND ROWNUM =1) AS Departamento,
    ...
    (SELECT SUM(a.ARTN_CANTIDAD_TOTAL) FROM pv_vta_diaria_articulo a WHERE a.artc_articulo = com_articulos.artc_articulo AND ticn_aaaammddventa BETWEEN '$fecha_inicio' AND '$fecha_fin' AND TICC_SUCURSAL = $sucursal) AS U_Vendidas, 
    (SELECT SUM(a.ARTN_VENTA_TOTAL) FROM pv_vta_diaria_articulo a WHERE a.artc_articulo = com_articulos.artc_articulo AND ticn_aaaammddventa BETWEEN '$fecha_inicio' AND '$fecha_fin' AND TICC_SUCURSAL = $sucursal) AS cantidad,
...
WHERE  COM_ARTICULOS.ARTN_ESTATUS=1 AND (SELECT SUM(a.ARTN_CANTIDAD_TOTAL) FROM pv_vta_diaria_articulo a WHERE a.artc_articulo = com_articulos.artc_articulo AND ticn_aaaammddventa BETWEEN '$fecha_inicio' AND '$fecha_fin' AND TICC_SUCURSAL = $sucursal) IS NOT NULL ORDER BY fam.famc_descripcion ASC";
...
// 	error_reporting(E_ALL);
	...
	while($row_principal = oci_fetch_row($consulta_principal))
	{
		...
		$cadena_sxrob = "SELECT SUM(INV_RENGLONES_MOVIMIENTOS.RMON_CANTSURTIDA) AS SXROB
		...
		AND INV_RENGLONES_MOVIMIENTOS.ARTC_ARTICULO = '$row_principal[2]'
		...
		GROUP BY  INV_RENGLONES_MOVIMIENTOS.ARTC_ARTICULO, INV_RENGLONES_MOVIMIENTOS.RMON_CANTSURTIDA
		ORDER BY ARTC_ARTICULO ASC";

			$consulta_sxrob = oci_parse($conexion_central, $cadena_sxrob);
			oci_execute($consulta_sxrob);
			$row_sxrob = oci_fetch_row($consulta_sxrob);

		$cadena_sxmbod = "SELECT SUM(INV_RENGLONES_MOVIMIENTOS.RMON_CANTSURTIDA) AS SXMBOD
		FROM INV_MOVIMIENTOS 
		INNER JOIN INV_RENGLONES_MOVIMIENTOS ON INV_MOVIMIENTOS.MODN_FOLIO = INV_RENGLONES_MOVIMIENTOS.MODN_FOLIO
		AND INV_MOVIMIENTOS.MODC_TIPOMOV = INV_RENGLONES_MOVIMIENTOS.MODC_TIPOMOV
		...
		ORDER BY ARTC_ARTICULO ASC";
			...
			$consulta_sxmbod = oci_parse($conexion_central, $cadena_sxmbod);
			oci_execute($consulta_sxmbod);
			$row_sxmbod = oci_fetch_row($consulta_sxmbod);

		$cadena_sxmcar = "SELECT SUM(INV_RENGLONES_MOVIMIENTOS.RMON_CANTSURTIDA) AS SXMCAR
		FROM INV_MOVIMIENTOS 
		INNER JOIN INV_RENGLONES_MOVIMIENTOS ON INV_MOVIMIENTOS.MODN_FOLIO = INV_RENGLONES_MOVIMIENTOS.MODN_FOLIO
		AND INV_MOVIMIENTOS.MODC_TIPOMOV = INV_RENGLONES_MOVIMIENTOS.MODC_TIPOMOV
		...
		ORDER BY ARTC_ARTICULO ASC";
			...
			$consulta_sxmcar = oci_parse($conexion_central, $cadena_sxmcar);
			oci_execute($consulta_sxmcar);
			$row_sxmcar = oci_fetch_row($consulta_sxmcar);

		$cadena_sxmfci = "SELECT SUM(INV_RENGLONES_MOVIMIENTOS.RMON_CANTSURTIDA) AS SXMFCI
		FROM INV_MOVIMIENTOS 
		INNER JOIN INV_RENGLONES_MOVIMIENTOS ON INV_MOVIMIENTOS.MODN_FOLIO = INV_RENGLONES_MOVIMIENTOS.MODN_FOLIO
		AND INV_MOVIMIENTOS.MODC_TIPOMOV = INV_RENGLONES_MOVIMIENTOS.MODC_TIPOMOV
		...
		ORDER BY ARTC_ARTICULO ASC";
			...
			$consulta_sxmfci = oci_parse($conexion_central, $cadena_sxmfci);
			oci_execute($consulta_sxmfci);
			$row_sxmfci = oci_fetch_row($consulta_sxmfci);

		$cadena_sxmfta = "SELECT SUM(INV_RENGLONES_MOVIMIENTOS.RMON_CANTSURTIDA) AS SXMFTA
		FROM INV_MOVIMIENTOS 
		INNER JOIN INV_RENGLONES_MOVIMIENTOS ON INV_MOVIMIENTOS.MODN_FOLIO = INV_RENGLONES_MOVIMIENTOS.MODN_FOLIO
		AND INV_MOVIMIENTOS.MODC_TIPOMOV = INV_RENGLONES_MOVIMIENTOS.MODC_TIPOMOV
		...
		ORDER BY ARTC_ARTICULO ASC";
			...
			$consulta_sxmfta = oci_parse($conexion_central, $cadena_sxmfta);
			oci_execute($consulta_sxmfta);
			$row_sxmfta = oci_fetch_row($consulta_sxmfta);

		$cadena_sxmpan = "SELECT SUM(INV_RENGLONES_MOVIMIENTOS.RMON_CANTSURTIDA) AS SXMPAN
		FROM INV_MOVIMIENTOS 
		INNER JOIN INV_RENGLONES_MOVIMIENTOS ON INV_MOVIMIENTOS.MODN_FOLIO = INV_RENGLONES_MOVIMIENTOS.MODN_FOLIO
		AND INV_MOVIMIENTOS.MODC_TIPOMOV = INV_RENGLONES_MOVIMIENTOS.MODC_TIPOMOV
		...
		ORDER BY ARTC_ARTICULO ASC";
			...
			$consulta_sxmpan = oci_parse($conexion_central, $cadena_sxmpan);
			oci_execute($consulta_sxmpan);
			$row_sxmpan = oci_fetch_row($consulta_sxmpan);
		...
		//$objPHPExcel->getActiveSheet()->getStyle('P'.$fila)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE);
		 $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('J'.$fila, $row_sxrob[0])
                ->setCellValue('K'.$fila, $row_sxmbod[0])
                ->setCellValue('L'.$fila, $row_sxmcar[0])
                ->setCellValue('M'.$fila, $row_sxmfci[0])
                ->setCellValue('N'.$fila, $row_sxmfta[0])
                ->setCellValue('O'.$fila, $row_sxmpan[0]);
