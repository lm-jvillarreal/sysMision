...
$sucursal = $_POST['sucursal_SIS002'];
$departamento = $_POST['departamento_SIS002'];
$variable = $_POST['jObject'];
$array = $_POST['array_SIS002'];
$proveedor = $_POST["proveedor_SIS002"];
...
if ($departamento == "" && $proveedor == "") {
		$cantidad = count($arra);
	for ($i=1; $i < $cantidad; $i++) { 
		$consulta = " OR com_articulos.ARTC_ARTICULO = '$arra[$i]'";
		$or = $or . $consulta;
	}
	$where = " WHERE
	(
		com_articulos.ARTC_ARTICULO = '$arra[0]'".$or."
	)";
	...
	$where =  " WHERE (SELECT PROC_CVEPROVEEDOR FROM com_articuloslistaprecios lista WHERE lista.artc_articulo = INV_ARTICULOS_DETALLE.artc_articulo AND rownum = 1) = '$proveedor'";
}
// $codigo = $_POST['codigo'];
// if ($codigo =="") {
// 	$filtro_codigo = "";
...
$objPHPExcel->getActiveSheet()
	...
	header('Content-Disposition: attachment;filename="Cambio_de_precios_ '.$fecha.' .xlsx"');
