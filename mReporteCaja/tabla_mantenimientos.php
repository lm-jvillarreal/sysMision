<?php 
  ...
  $cadena  = "SELECT id,
                ...
                (SELECT nombre FROM cajas_catalogo_equipos WHERE cajas_catalogo_equipos.id = cajas_mantenimiento_equipos.id_equipo),comentario
  ...
  while ($row = mysqli_fetch_array($consulta)) 
  {
    ...
    $total_imagenes = count(glob("imagesA/".$row[0]."/{*.jpg,*.jpeg,*.png}",GLOB_BRACE));
    for ($i=1; $i < $total_imagenes ; $i++) { 
      if (file_exists('imagesA/'.$row[0].'/'.$i.'.jpg')){
        // echo "si";
        $ruta = 'imagesA/'.$row[0].'/'.$i.'.jpg';
        $imagenes .= "<a class='venobox hidden' href='$ruta' alt='Imagen $i' data-gall='myGallery_$numero' title='$texto'>$i</a>";
      }else{
        // echo "no";
        $ruta = 'imagesA/'.$row[0].'/'.$i.'.jpeg';
        $imagenes .= "<a class='venobox hidden' href='$ruta' alt='Imagen $i' data-gall='myGallery_$numero' title='$texto'>$i</a>";
      }
    }
    if (file_exists('imagesA/'.$row[0].'/0.jpg')){
      $ruta2 = 'imagesA/'.$row[0].'/0.jpg';
      $color = "success";
    }else if(file_exists('images/'.$row[0].'/0.jpeg')){
      $ruta2 = 'imagesA/'.$row[0].'/0.jpeg';
      $color = "success";
    }else{
      $color = "warning";
    }
