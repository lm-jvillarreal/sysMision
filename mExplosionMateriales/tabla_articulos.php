...
$datos=array();
$porcetaje = $_POST["porcentaje"];
//Obtener la lista de recetas registradas en el sistema
$cadenaRecetas="SELECT ID, ARTC_ARTICULO, ARTC_DESCRIPCION FROM panaderia_recetasventa WHERE ACTIVO=1";
...
while($rowRecetas=mysqli_fetch_array($consultaRecetas)){
  ...
  while($rowDetalle=mysqli_fetch_array($consultaDetalle)){
    if($rowDetalle[1]=='1'){
      ...
      $cadenaSR="SELECT ID, UNIDAD_MEDIDA, RENDIMIENTO AS FACTOR_EMPAQUE FROM panaderia_subrecetas WHERE CLAVE_RECETA='$rowDetalle[0]'";
      ...
      $cadenaDesglose="SELECT ID, ID_ARTICULO, CANTIDAD_RECETA, SUBRECETA FROM panaderia_subrecetasrenglones WHERE ID_SUBRECETA='$rowSR[0]' AND ACTIVO='1'";
  ...
  $porcetaje_real = $porcetaje / 100;
  //Agregadas para verificar IEPS
  $sinIepsSg = round($costoNeto / (1-$porcetaje_real), 2);
  $sinIepsGrlCI = round($conIva / 1.08, 2);
  array_push($datos,array(
    'platillo'=>$rowRecetas[2],
    'sin_ieps'=> $sinIepsGrlCI,
    'costo'=>$costoNeto,
    'ieps_sugerido'=>$sinIepsSg,
    'con_iva'=>$conIva,
    'cv'=>$cv,
    'mb'=>$mb,
    'utilidad'=>$utilidad
