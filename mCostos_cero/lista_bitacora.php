...
$cadena_codigos = "SELECT c.id, c.codigo, c.articulo, c.costo, s.nombre, c.comentario, c.baja, c.estatus, u.nombre_usuario
                        FROM costos_cero as c
                        INNER JOIN sucursales as s ON c.sucursal = s.id 
                        INNER JOIN usuarios as u ON c.id_resuelve = u.id WHERE  c.estatus = '3'";
...
while ($row_codigos = mysqli_fetch_array($consulta_codigos)) {
    ...
    $baja = "<a href='#' class='btn btn-warning' onclick='baja($row_codigos[1])'$solo_lectura>Baja</a>";
    if($row_codigos[7]=='1'){
