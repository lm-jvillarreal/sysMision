...
$cadena_consulta  = "SELECT  DISTINCT(s.codigo),
					(SELECT SUM(cantidad) FROM detalle_solicitud WHERE detalle_solicitud.codigo = s.codigo AND detalle_solicitud.id_solicitud = '$id_solicitud') As 'Cantidad'
					FROM detalle_solicitud as s where s.id_solicitud = '$id_solicitud'";
...
// 	error_reporting(E_ALL);
	...
	$objPHPExcel->getProperties()->setCreator("Josué Villarreal")
								 ...
								 ->setTitle("Reporte de mermas")
								 ->setSubject("Reporte de mermas")
								 ->setDescription("Reporte de mermas")
	...
	//Rename worksheet
	$objPHPExcel->getActiveSheet()->setTitle('Rep. Merma');
	...
	header('Content-Disposition: attachment;filename="Merma_'.$fecha.'.xlsx"');
