<?php
  ...
  $cadena = mysqli_query($conexion,"SELECT id_usuario,( SELECT CONCAT(nombre,' ',ap_paterno,' ', ap_materno) FROM personas WHERE usuarios.id_persona = personas.id), usados,actual
    FROM vacaciones
    INNER JOIN usuarios ON usuarios.id = vacaciones.id_usuario");
